...
window.onload = function(){
	waterfall('main','pin');
	var dataInt={'data':[{'src':'1.jpg'},{'src':'2.jpg'},{'src':'3.jpg'},{'src':'4.jpg'},{'src':'5.jpg'},{'src':'6.jpg'},{'src':'7.jpg'},{'src':'8.jpg'},{'src':'9.jpg'},{'src':'10.jpg'},{'src':'1.jpg'},{'src':'2.jpg'}]};
	window.onscroll=function(){
	    if(checkscrollside()){
	        var main=document.getElementById('main');// 父级对象
	        for(var i=0;i<dataInt.data.length;i++){
	            var pin=document.createElement('div'); //创建添加 元素节点pin
	            pin.className='pin';                   //添加 类名 name属性
	            main.appendChild(pin);              //创建添加 子节点box
	            var box=document.createElement('div');
	            box.className='box';
	            pin.appendChild(box);
	            var img=document.createElement('img');//创建添加 子节点img
	            img.src='./imgs/'+dataInt.data[i].src;
	            box.appendChild(img);
	        }
	        waterfall('main','pin');//封装成函数waterfall()，将添加的节点添加到添加和定位到文档中。
	    };
	}
}
